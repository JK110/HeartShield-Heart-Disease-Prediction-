document.addEventListener("DOMContentLoaded", () => {
    ...
    if(manualForm) {
        manualForm.addEventListener('submit', async (e) => {
            ...
            if(manualForm.checkValidity()){
                ...
                showStatus(resultText, "Please fill all required fields.", "error", resultsSection);
            }
        });
    ...
    async function handleReportUpload() {
        ...
        try {
            ...
            showStatus(uploadStatus, 'Data extracted. Review fields and click "Analyse Risk".', 'success');
    ...
    function populateForm(data) {
        ...
        setVal('age', has(data.age) ? data.age : (has(profile.age) ? profile.age : ''));
        setVal('height', has(data.height) ? data.height : (has(profile.height) ? profile.height : ''));
        setVal('weight', has(data.weight) ? data.weight : (has(profile.weight) ? profile.weight : ''));
        
        if (has(data.gender) || has(profile.gender)) {
            const val = (data.gender || profile.gender).toLowerCase();
            document.getElementById('gender').value = val.startsWith('m') ? 'male' : 'female';
        }
    ...
    async function startCamera() {
        fileInput.value = ''; handleRemoveFile();
        try {
    ...
    function showPreview(file) {
        ...
        if(file.type === 'application/pdf') {
            ...
            f.src = url; f.style.width='100%'; f.style.height='500px'; f.style.border='none';
            previewContent.appendChild(f);
        } else {
            const i = document.createElement('img');
            i.src = url; i.style.maxWidth='100%'; i.style.display='block'; i.style.margin='0 auto';
            previewContent.appendChild(i);
        }
    }
    
    function handleRemoveFile() {
        fileInput.value = ''; capturedFile = null; 
        previewContainer.style.display='none'; previewContent.innerHTML=''; uploadStatus.style.display='none';
        stopCamera();
        resultsSection.style.display = 'none';
        hospitalSection.style.display = 'none';
        resultText.innerHTML = '';
    ...
    function displayResults(result) {
        ...
        let color = result.probability > 75 ? '#dc3545' : (result.probability > 50 ? '#FFB74D' : '#28a745');
        let text = result.probability > 75 ? 'High Risk' : (result.probability > 50 ? 'Moderate Risk' : 'Low Risk');
    ...
    function updateHospitalLink() {
        ...
        const defaultUrl = "https://www.google.com/maps/search/cardiologist+near+me";
        btn.href = defaultUrl;
    ...
    function showLoadingOverlay() {
        ...
        quoteInterval = setInterval(() => {
            ...
            setTimeout(() => { loadingQuote.textContent = quotes[i]; loadingQuote.style.opacity = 1; }, 300);
        }, 2500);
    }
    ...
    async function handleFeedbackSubmit(e) {
        ...
        if (!review) { showStatus(statusDiv, 'Please enter a review.', 'error'); return; }
